		...
		<div id="content">
			<div id="board">
				...
				<table class="tbl-ex">
					...
					<c:set var="totalCount" value="${fn:length(list) }"></c:set>
					<c:forEach var="vo" items="${list }" varStatus="status">
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${not empty authUser && authUser.no == vo.usersNo }">
...
<c:if test = '${param.m == "needlogin" }'>
	<script>
		alert("로그인이 필요한 서비스 입니다..");
	</script>
</c:if>
<c:if test = '${param.m == "success" }'>
	<script>
		alert("저장이 완료되었습니다.");
	</script>
</c:if>
<c:if test = '${param.m == "delsuccess" }'>
	<script>
		alert("삭제되었습니다.");
	</script>
</c:if>