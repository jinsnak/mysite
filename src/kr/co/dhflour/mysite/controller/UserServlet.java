...
public class UserServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		} else if ("joinsuccess".equals(actionName) == true) {
			RequestDispatcher rd =  request.getRequestDispatcher("/WEB-INF/views/user/joinsuccess.jsp");
			rd.forward(request, response);
		
		} else if ("loginform".equals(actionName) == true) {
			RequestDispatcher rd =  request.getRequestDispatcher("/WEB-INF/views/user/loginform.jsp");
			rd.forward(request, response);
			
		} else if ("login".equals(actionName) == true) {
			String email = request.getParameter("email");
			String password = request.getParameter("password");
		...
		} else if ("logout".equals(actionName) == true) {
		...
		} else if ("modifyform".equals(actionName) == true) {
			HttpSession session = request.getSession();
			if (session == null) {
				response.sendRedirect("/mysite/main");
				return;
		...
		} else if ("modify".equals(actionName) == true) {
			HttpSession session = request.getSession();
			if (session == null) {
				response.sendRedirect("/mysite/main");
				return;
